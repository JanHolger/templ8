...
import eu.bebendorf.purejavaparser.ast.expression.GreaterThanOrEqual;
import eu.bebendorf.templ8.eval.Scope;

public class GreaterThanOrEqualOperation implements NumericOperation {
    ...
    public NumberPair pair(Scope scope, Expression expression) throws Exception {
        if(!(expression instanceof GreaterThanOrEqual))
            return null;
        return NumberPair.make(((GreaterThanOrEqual) expression).getFirst(), ((GreaterThanOrEqual) expression).getSecond());
    }
