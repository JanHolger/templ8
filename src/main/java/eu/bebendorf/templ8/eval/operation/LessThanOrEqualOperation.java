...
import eu.bebendorf.purejavaparser.ast.expression.LessThanOrEqual;
import eu.bebendorf.templ8.eval.Scope;

public class LessThanOrEqualOperation implements NumericOperation {
    ...
    public NumberPair pair(Scope scope, Expression expression) throws Exception {
        if(!(expression instanceof LessThanOrEqual))
            return null;
        return NumberPair.make(((LessThanOrEqual) expression).getFirst(), ((LessThanOrEqual) expression).getSecond());
    }
