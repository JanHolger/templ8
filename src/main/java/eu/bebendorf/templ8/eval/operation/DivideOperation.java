...
import eu.bebendorf.purejavaparser.ast.expression.Division;
import eu.bebendorf.purejavaparser.ast.expression.Expression;
import eu.bebendorf.templ8.eval.Scope;

public class DivideOperation implements NumericOperation {
    ...
    public NumberPair pair(Scope scope, Expression expression) throws Exception {
        if(!(expression instanceof Division))
            return null;
        return NumberPair.make(((Division) expression).getDividend(), ((Division) expression).getDivisor());
    }
